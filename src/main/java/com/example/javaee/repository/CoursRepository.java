...
public interface CoursRepository extends JpaRepository<Cour, CourId> {
    ...
    // Méthode pour récupérer les cours d'une matière
    List<Cour> findByMatiere(Matiere matiere);

    // Trouver tous les cours associés à une matière (en utilisant une inscription)
    @Query("SELECT c FROM Cour c WHERE c.matiere IN (" +
    "SELECT i.matiere FROM Inscription i WHERE i.etudiant = :etudiant)")
    List<Cour> findByEtudiant(@Param("etudiant") Utilisateur etudiant);
